...
Openvino example: https://github.com/openvinotoolkit/openvino_notebooks/blob/375819db2038a4f4e22e5ba65bcf9d7e4ad19abd/notebooks/yolov8-optimization/yolov8-instance-segmentation.ipynb
"""
...
class YOLOv8DetectorSegmenter:
    ...
    def postprocess(self, pred_boxes:np.ndarray, input_hw:Tuple[int, int], orig_img:np.ndarray, min_conf_threshold:float = 0.25,
        nms_iou_threshold:float = 0.7, agnosting_nms:bool = False, max_detections:int = 300, pred_masks:np.ndarray = None, retina_mask:bool = False):
        """
        ...
        preds = ops.non_max_suppression(torch.from_numpy(pred_boxes), min_conf_threshold, nms_iou_threshold, nc=len(self.cls_names), **nms_kwargs)
        results = []
    ...
    def main(self, image_path, output_image):
        try:
            ...
            preprocessed_image = ops.processing_image(image)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_path", default="./debug/result" ,type=str)
    parser.add_argument("--output_image", type=str, default="result.jpg")
    args = parser.parse_args() 

    model = YOLOv8DetectorSegmenter(args.model_path, cls_names = ["airpod", "appleW", "netgrear", "phone"], threshold= 0.8, output_path=args.output_path)
    model.main(args.image_path, args.output_image)